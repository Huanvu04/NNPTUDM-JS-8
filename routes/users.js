...
router.post("/import", uploadExcel.single('file'), async function (req, res, next) {
    try {
        if (!req.file) {
            return res.status(400).send({ message: "Vui lòng tải lên file Excel!" });
        }     
        // Gọi hàm Import từ Controller
        let result = await userController.ImportUserFromExcel(req.file.path);
        res.send(result);
    } catch (err) {
